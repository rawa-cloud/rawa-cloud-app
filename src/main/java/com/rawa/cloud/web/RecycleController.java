...
//    }
    ...
    @ApiOperation("查询")
    @GetMapping("")
    public JsonResult<List<Recycle>> query(@Valid RecycleQueryModel model, BindingResult result) {
    ...
    @ApiOperation("批量添加")
    @PostMapping("/batch")
    public JsonResult<List<Long>> addInBatch(@Valid @RequestBody RecycleAddInBatchModel model, BindingResult result) {
        ValidationHelper.validate(result);
        return JsonResult.success(recycleService.addInBatch(model));
    }
